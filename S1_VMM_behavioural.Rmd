...
if (!file.exists(file.path(model_dir, "VMM_data.RData"))) {
  ...
  # load information on medication
  df.meds = read_csv(file.path("/home/emba/Documents/EMBA/CentraXX", 
                               "VMM_medication.csv"))
  ...
  df.sublng = df.sub %>%
    ...
    select(diagnosis, Age, `IQ estimate`, `ASRS-v1.1`, `RADS-R`, BDI, Education) %>%
  ...
  for (m in unique(df.sublng$name)) {
    ...
    # overwrite results with table in the case of education
    if (m == "Education") {
      aov@bayesFactor[["bf"]] = ct.edu@bayesFactor$bf
    }
  ...
  # save it all
  save(df.disc, df.tsk, df.fix.agg, tb.screen, 
       ct.full, ct.mf, gen.desc, df.meds, tb.edu, ct.edu,
