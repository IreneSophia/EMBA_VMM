...
if [ $pooled == 1 ]; then
	...
	randomise -i "$dir_out/fMRI_data/$code" -o "$dir_ostt/hgf_all_mu_c" -d "$dir_out/fMRI_designs/glm_ostt-cov-f.mat" -t "$dir_out/fMRI_designs/glm_ostt-cov-f.con" -f "$dir_out/fMRI_designs/glm_ostt-cov-f.fts" -m fMRI_data/WB_mask -1 -T -n $nsim &
