...
function [heatmapRGB,heatmap]=heatmap_generator_ISP(records,scene,spacing,max_hor,max_ver,kernel_size_gaussian,s_gaussian)

data_struct=load(records);
f=fieldnames(data_struct);
if ~isempty(scene)
    scene = imread(scene);
else
    scene = ones(max_ver, max_hor, 3,'uint8');
end
mtx=data_struct.(f{1});
x=mtx(:,1);
y=mtx(:,2);
